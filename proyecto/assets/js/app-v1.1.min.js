...
function pasteSelection() {
    const offsetX = 0; // Ejemplo de desplazamiento horizontal
    const offsetY = 0; // Ejemplo de desplazamiento vertical
...
// Función para crear un joint en las coordenadas ajustadas a la cuadrícula
function createJoint(x, y) {
    const canvas = document.getElementById('main-canvas');
    const ctx = canvas.getContext('2d');

    // Dibuja un círculo en la posición ajustada
    ctx.beginPath();
    ...
    console.log(`Joint created at: (${x}, ${y})`);
}

// Manejador de eventos para clics en el botón "Add"
document.getElementById('add-button').addEventListener('click', function() {
    creationMode = true;
    console.log('Creation mode on');
});

// Manejador de eventos para clics en el canvas
document.getElementById('main-canvas').addEventListener('click', function(event) {
    ...
    // Ajusta las coordenadas a la cuadrícula antes de crear el joint
    const [snappedX, snappedY] = snapToGrid(x, y, gridSize);

    // Crea el joint usando las coordenadas ajustadas
    createJoint(snappedX, snappedY);

    // Desactiva el modo de creación (opcional)
    // creationMode = false;
    console.log('Joint created and mode deactivated');
});
...
class Beam {
    ...
    calculateLength() {
        return (Math.sqrt(Math.pow(this.jointB.position[0] - this.jointA.position[0], 2) + 
                         Math.pow(this.jointB.position[1] - this.jointA.position[1], 2)));
    }
    
    calculateWeight() {
        const BEAM_WEIGHT_PER_M = 20.4; // kg/m
        return this.length * BEAM_WEIGHT_PER_M;
    }
    
    calculateStress() {
        const force = this.calculateWeight() * 9.81; // Convertir el peso en fuerza (N)
        const crossSectionalArea = 1; // cm², asumir área de sección transversal
        return force / crossSectionalArea; // Stress (N/cm²)
    }
...
touchController.clicked = ((e,t)=>{
    ...
    if (e)
        e instanceof Joint ? (beamEditor.disable(),
    
        jointEditor.enableForJoint(e)) : e instanceof Beam && (jointEditor.disable(),
        beamEditor.enableForBeam(e));
        
        
    else if (creationMode) {
        let snappedPosition = snapToGrid(t[0], t[1], gridSize); // Ajuste a la cuadrícula
        let e = new Joint(renderEngine);
        bridgeModel.joints.push(e),
        e.updatePosition(snappedPosition, { ignoreLock: !0 }), // Usar posición ajustada
        renderEngine.requestUpdate()
        history.addActionToHistory(new AddJointAction(e));//historial
...
touchController.dragEnded = ((e,t)=>{
    let i = renderEngine.clipPosition(t);
    let snappedPosition = snapToGrid(i[0], i[1], gridSize); // Ajuste a la cuadrícula
    var o = renderEngine.jointAtPosition(snappedPosition, e); // Usar posición ajustada
    if (creationMode) {
        if (!bridgeModel.temporaryJoint) return;
        bridgeModel.temporaryJoint.updatePosition(snappedPosition), // Usar posición ajustada
        bridgeModel.temporaryJoint.moving = !1,
        o ? bridgeModel.temporaryBeam.jointA != o && (bridgeModel.temporaryBeam.jointB = o,
        bridgeModel.beams.push(bridgeModel.temporaryBeam),
        bridgeModel.mergeRedundantBeams(bridgeModel.temporaryBeam.jointA, o),
        renderEngine.requestUpdate()) : (bridgeModel.addJoint(bridgeModel.temporaryJoint),
        bridgeModel.beams.push(bridgeModel.temporaryBeam),
        renderEngine.requestUpdate()),

        history.addActionToHistory(new AddJointAction(bridgeModel.temporaryJoint));//historial
        history.addActionToHistory(new AddBeamAction(bridgeModel.temporaryBeam));//historial
        ...
        e && (e.updatePosition(snappedPosition), // Usar posición ajustada
        e.moving = !1,
        o && bridgeModel.mergeJoint(o, e),
        renderEngine.requestUpdate())
...
function createArchBridge(length) {
    ...
    for (let i = 0; i <= numSegments; i++) {
        ...
        // Conectar el arco a la carretera con vigas verticales y diagonales, para solo vigas verticales i+1!!!
        
        if (i > 0 && i < numSegments) {
            addBeam(bridgeModel.joints[i], roadJoint);
            addBeam(bridgeModel.joints[i+1], roadJoint);
            addBeam(bridgeModel.joints[i+2], roadJoint);
...
function createPrattBridge(bridgeLength) {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        const vBeamJointB = addJoint([vBeamX, centerY - pillarHeight]);
    ...
    const firstVerticalJointTop = addJoint([startX + vBeamSeparation, centerY - pillarHeight]);
    const lastVerticalJointTop = addJoint([startX + 9 * vBeamSeparation, centerY - pillarHeight]);
...
bridgeModel.joints = bridgeModel.joints.filter(joint => 
    !(joint.position[0] === startX + 10 * vBeamSeparation && joint.position[1] === centerY - pillarHeight)
);

// Eliminar los beams que involucran al joint superior de jointB
bridgeModel.beams = bridgeModel.beams.filter(beam =>
    !(
        (beam.jointA.position[0] === startX + 10 * vBeamSeparation && beam.jointA.position[1] === centerY - pillarHeight) ||
        (beam.jointB.position[0] === startX + 10 * vBeamSeparation && beam.jointB.position[1] === centerY - pillarHeight)
    )
);
...
function generateBridge() {
    const distance = prompt("Please enter the distance for the bridge (in meters):");
...
function checkStability() {
    ...
    const MAX_BEAM_LENGTH = 5000; // Longitud máxima permisible para una viga en mm
    const MAX_BEAM_MOMENT= 50000000;
    ...
    const VEHICLE_WEIGHT = 15000; // kg, peso promedio de un vehículo
    ...
    const Y_MAX = 0.05; // Distancia máxima al eje neutro (m) para HEB 100
    const E = 210000000000; // Módulo de elasticidad para el acero (Pa)
    const I = 8.17e-6; // Momento de inercia (m^4) para HEB 100
    ...
    beams.forEach(beam => {
        ...
        let slenderness = length / (BEAM_WEIGHT_PER_M / 7850); // Ratio de esbeltez (longitud/radio de giro)
        ...
        if (maxDeflection > length / 200) { // Límite arbitrario para la deflexión
            console.log(`Beam between (${jointA.position[0]}, ${jointA.position[1]}) and (${jointB.position[0]}, ${jointB.position[1]}) has deflection ${maxDeflection} m which is too high.`);
            stable = false;
        }
    });

    // Verificar la presencia de triángulos en la estructura
    let triangles = findTriangles(joints, beams);
    if (triangles.length < beams.length / 3) {
        console.log(`The bridge does not have enough triangles to ensure stability.`);
        stable = false;
    }

    // Verificar ángulos de los triángulos
    triangles.forEach(triangle => {
        let [jointA, jointB, jointC] = triangle;
        ...
        if (angleAB_AC < MIN_ANGLE || angleAB_AC > MAX_ANGLE ||
            angleBA_BC < MIN_ANGLE || angleBA_BC > MAX_ANGLE ||
            angleCA_CB < MIN_ANGLE || angleCA_CB > MAX_ANGLE) {
...
function findTriangles(joints, beams) {
    ...
    // Helper function to check if a beam exists between two joints
    function beamExists(jointA, jointB) {
    ...
    // Iterate over all unique sets of three joints
    for (let i = 0; i < joints.length; i++) {
        for (let j = i + 1; j < joints.length; j++) {
            for (let k = j + 1; k < joints.length; k++) {
                ...
                // Check if these three joints form a triangle
...
function distributeTruckLoad(centerIndex, initialLoad) {
    ...
    distributeLoad(centerBeam, initialLoad, visitedBeams, visitedJoints);

    return initialLoad;
}

function distributeLoad(beam, load, visitedBeams = new Set(), visitedJoints = new Set()) {
    ...
    distributeLoadToJoint(beam.jointA, totalLoad / 2, visitedJoints);
    distributeLoadToJoint(beam.jointB, totalLoad / 2, visitedJoints);
    ...
    const distributedLoad = totalLoad / connectedBeams.length;
    connectedBeams.forEach(connectedBeam => {
        distributeLoad(connectedBeam, distributedLoad, visitedBeams, visitedJoints);
    });
}

function distributeLoadToJoint(joint, load, visitedJoints) {
    if (visitedJoints.has(joint) || load < 1) return;
    visitedJoints.add(joint);
    if (joint.type !== 'f') {
        joint.appliedLoad += load;
    }
}
...
function simulateWithTruck() {
    ...
    distributeLoad(bridgeModel.beams[centerIndex], initialLoad);
...
const FRAME_RATE = 30; // Tasa de frames por segundo
...
const STRENGTH_LIMIT1 = 30000; // Límite de resistencia para la coloración
const UPDATE_INTERVAL = 500; // Intervalo de actualización en ms
...
function distributeLoadToAdjacentBeams(beam, x, y, depth = 0, maxDepth = 5, attenuationFactor = 0.1) {
    ...
    const loadPerBeam = ADDED_LOAD * Math.pow(attenuationFactor, depth) / (connectedBeams.length + 1); // +1 para incluir el beam actual
...
function simulateWithTruck5(startJoint, endJoint, duration) {
    ...
    const BEAM_CHANGE_THRESHOLD = 5000; // Tiempo en ms para detectar un cambio de beam (5 segundos)
    ...
    function animateTruck() {
        ...
        if (closestBeam) {
            if (closestBeam !== lastBeam) {
                ...
                // Si ha pasado mucho tiempo en el mismo beam, pasar al siguiente
                console.log('End');
                return;
            }
...
function updateLoad() {
    ...
    if (timeElapsed >= UPDATE_INTERVAL) {
        bridgeModel.beams.forEach(beam => {
            if (beam.appliedLoad !== undefined) {
                const reductionFactor = ADDED_LOAD * (timeElapsed / 4000); // Ajuste de la reducción por segundo
...
function animateBridge() {
    ...
    if (fixedJoints.length < 2) {
        console.error('El puente necesita al menos dos joints fijos.');
        return;
    }
